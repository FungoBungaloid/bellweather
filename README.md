...
> the elasticity is a national average applied to each market's local anomaly.

### Making it real on GitHub Pages (no local run needed)

GitHub Pages serves **static files only** — it can't run Python, so `calibrate.py`
can't execute "on the page". Instead it runs in **CI** and commits the results, which
Pages then serves:

1. The committed **`.github/workflows/build-data.yml`** runs on demand
   (**Actions → Build data → Run workflow**) and on a monthly cron. On a runner —
   which *does* have network and can send the required `User-Agent` — it:
   - fetches the world geography (`land-110m.json`, `countries-110m.json`,
     `states-10m.json`) so the **globe coastlines and regional map outlines** appear, and
   - runs `python scripts/calibrate.py` (real ERA5 + Wikipedia pageviews) to overwrite
     `data/coefficients.json` and `data/normals.json`.
   - then commits those files back to the branch.
2. Point **Settings → Pages** at that same branch (`/root`). When the workflow's commit
   lands, Pages redeploys and the live site now has **real calibrated elasticities/normals
   and real map outlines** — no local steps required.

(The map geography is blocked from the dev sandbox by network policy, which is why the
committed repo ships the city-constellation globe + graticule until the workflow runs.)
